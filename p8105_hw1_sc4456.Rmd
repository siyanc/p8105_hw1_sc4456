...
```
The size of of the dataset is `r dim(data_table2)`, the mean of x is `r mean(x)`, the median of x is `r median(x)`, the proportion of cases for which the logical vector is TRUE IS `r sum(var_numeric)/length(var_numeric)`
